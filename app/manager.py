...
class Manager:
    ...
    async def run(self, query: str) -> str:
        ...
        self.logger.info(f"Generated execution plan: {json.dumps([t.model_dump() for t in plan.tasks], ensure_ascii=False)}")
        
        # Extract tasks from plan
        tasks = plan.tasks
        if not tasks:
            self.logger.warning("Unable to generate a valid execution plan for the user query")
            return "Unable to generate a valid execution plan for your question."
        
        # 2. Create execution context
        self.logger.info(f"Creating execution context, number of tasks: {len(tasks)}")
        ...
        while not context.finished and task_index < len(tasks):
            ...
            self.logger.info(f"Executing task {task_index + 1}/{len(tasks)}: {context.current_task.task}")
            ...
            self.logger.debug(f"Executor agent result: {executor_result.final_output}")
            ...
            self.logger.info(f"Evaluation status: {eval_output.status}, next action: {eval_output.action}")
            ...
            if eval_output.status == "completed":
                ...
                self.logger.info(f"Task {task_index + 1} completed, continuing execution plan")
                results.append(f"Task {task_index + 1} completed: {context.current_task.task}")
                task_index += 1
                ...
                self.logger.info(f"Adjusted execution plan: {json.dumps([t.model_dump() for t in new_plan.tasks], ensure_ascii=False)}")
                ...
                if not tasks:
                    self.logger.warning("Adjusted plan contains no tasks, terminating execution")
                ...
                # Terminate execution
                self.logger.warning(f"Terminating execution: {eval_output.summary}")
                context.finished = True
