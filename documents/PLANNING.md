...
核心能力:
  ...
  - 团队领导: 统筹Monica和Henry的工作，确保团队目标一致

团队领导者特性:
  - 唯一性保证: 每个团队仅有一个Jeff实例，确保决策统一性
  - 领导决策权: 负责团队整体战略方向和重大决策
  - 优先级处理: 在任务调度中享有最高优先级
  - 不可扩展: Jeff实例数量固定为1，不参与自动扩展

规划触发场景:
  - 团队启动规划: 团队创建时自动触发初始战略规划和资源分配
  - 失败重新规划: 任务达到最大重试次数时自动触发重新规划流程  
  - 用户建议规划: 团队成员可提供建议触发特定或全局任务重新规划

自动化特性:
  - 实时数据分析: 自动收集和分析市场数据
  - 策略优化: 基于执行结果自动优化策略
  - 预测分析: AI 驱动的营销效果预测
  - 团队协调: 智能分配任务给Monica和Henry，优化团队效率
...
#### 🚀 场景一：团队启动规划（Team Kick-off Planning）

**触发时机**：团队创建完成，专家实例初始化后  
**触发方式**：自动触发，无需人工干预  
**执行目标**：为新团队制定初始工作策略和目标框架
...
class TeamManager:
    ...
    async def _trigger_team_kickoff_planning(self, team_id: str, creator_id: str):
        # 创建启动规划任务
        kickoff_task = await blackboard.create_task(
            title="团队启动规划",
            description="制定团队启动计划，包括目标、策略和资源分配",
            required_expert_role=ExpertRole.PLANNER,
            creator_id=creator_id
        )
        
        # 自动分配给Jeff（团队领导者）
        await blackboard.auto_assign_task(kickoff_task.task_id)
```

#### 🔄 场景二：失败重新规划（Task Failure Replanning）

**触发时机**：任务重试次数达到上限仍然失败  
**触发方式**：工作流引擎自动检测并触发  
**执行目标**：重新评估所有未完成任务，调整执行策略
    ...
    async def _handle_failed_tasks(self, workflow: WorkflowDefinition, failed_tasks: Set[str]):
        for task_id in failed_tasks:
            if node.retry_count >= node.max_retries:
                # 触发Jeff重新规划而非直接标记失败
                await team_manager.handle_task_max_retries_reached(
                    self.team_id, task_id, "workflow_engine"
                )
                
class TeamManager:
    async def handle_task_max_retries_reached(self, team_id: str, task_id: str):
        # 标记失败任务
        ...
        replanning_task = await blackboard.create_task(
            title=f"任务失败重新规划 - 基于失败任务 {task_id}",
            description="重新评估和规划所有未完成任务，制定新的执行策略",
            required_expert_role=ExpertRole.PLANNER,
            metadata={
...
#### 💡 场景三：用户建议规划（User Suggestion Replanning）

**触发时机**：团队成员主动提交优化建议  
**触发方式**：API调用，支持指定任务或全局规划  
**执行目标**：根据用户反馈优化特定任务或整体策略

```python
# API接口设计
@router.post("/teams/{team_id}/planning/user-suggestion")
async def submit_user_suggestion_replanning(
    ...
    # 创建用户建议规划任务
    result = await team_manager.handle_user_suggestion_replanning(
        team_id=team_id,
        user_id=user_id,
...
class TeamManager:
    async def handle_user_suggestion_replanning(
        self, team_id: str, user_id: str, suggestion_content: str, target_task_ids: List[str]
    ):
        # 验证用户权限
        is_team_member = any(member.user_id == user_id for member in team.members)
        
        # 获取目标任务（指定任务或所有未完成任务）
        target_tasks = await self._get_target_tasks(target_task_ids)
        
        # 创建用户建议规划任务
        replanning_task = await blackboard.create_task(
            title="用户建议重新规划",
            description=f"用户建议内容：{suggestion_content}",
            required_expert_role=ExpertRole.PLANNER,
            metadata={
...
#### 🔄 自动化触发机制
- **事件驱动**：基于团队生命周期事件自动触发
- **失败检测**：智能识别任务失败模式并触发重新规划
- **用户交互**：支持团队成员主动触发规划优化

#### 🧠 智能任务分配
- **领导者优先**：所有规划任务自动分配给Jeff（唯一team leader）
- **上下文传递**：完整的失败原因和用户建议上下文传递给Jeff
- **优先级管理**：规划任务享有高优先级，确保及时处理

#### 📊 执行结果跟踪
- **规划效果监控**：跟踪重新规划后的任务执行效果
- **学习优化**：基于规划结果持续优化触发策略
- **反馈循环**：形成"执行-反馈-优化-重新规划"的闭环

### 全自动化专家协作流程（增强版）
1. **🚀 团队创建** → 自动初始化专家实例和监控服务
2. **🎯 启动规划** → Jeff自动执行团队kick-off规划任务
3. **🔄 工作流启动** → 基于初始规划创建示例营销工作流
4. **🧠 智能调度** → 自动分析任务并分配给最合适专家
5. **📋 依赖管理** → 自动解析任务依赖关系和执行顺序
6. **✍️ 自动内容生成** → Monica自动根据策略创作内容
7. **🔍 自动内容审核** → Henry自动检查内容合规性
8. **🔄 失败重新规划** → 任务失败时Jeff自动重新规划策略
9. **💡 用户建议处理** → 响应用户建议进行策略优化
10. **📊 实时性能监控** → 持续监控执行状态和性能指标
11. **🔄 持续优化** → 基于执行结果自动优化调度策略
...
experts:
  jeff:
    name: "营销策略专家 & 团队领导者"
    max_instances: 1  # Jeff is unique team leader
    ...
    is_team_leader: true
    scaling_enabled: false  # Jeff never scales
