...
| 专家 | 角色 | 核心能力 | 自动化功能 | 实例数量 |
|------|------|----------|-----------|----------|
| **Jeff** | 🎯 **营销策略专家 & 团队领导者** | 制定营销目标、用户定位、渠道策略、预算规划、团队协调 | 自动策略生成、市场分析、团队决策 | **唯一实例（Team Leader）** |
| **Monica** | 内容生成专家 | 创作平台适配内容、运用营销技巧、多格式输出 | 自动内容创作、平台优化 | 可扩展（1-5个实例） |
| **Henry** | 内容审核专家 | 平台合规检查、地区法规验证、质量评估 | 自动合规检测、风险评估 | 可扩展（1-3个实例） |
...
**核心职责**：制定全面的营销策略和执行计划，担任团队唯一领导者

#### 🎯 团队领导者地位
- **唯一性保证**：每个团队仅有一个Jeff实例，确保决策统一性
- **领导决策权**：负责团队整体战略方向和重大决策
- **优先级处理**：在任务调度中享有最高优先级
- **团队协调**：统筹Monica和Henry的工作，确保团队目标一致

#### 🔄 三大规划触发场景
Jeff作为团队唯一领导者，在以下三种关键场景下自动执行规划任务：

1. **🚀 团队启动规划（Team Kick-off）**
   - 团队创建时自动触发初始战略规划
   - 制定团队目标、资源分配和执行策略
   - 为团队后续工作奠定基础框架

2. **🔄 失败重新规划（Task Failure Replanning）**
   - 当任务达到最大重试次数仍失败时自动触发
   - Jeff重新评估所有未完成任务的执行策略
   - 基于失败原因调整工作流和任务分配

3. **💡 用户建议规划（User Suggestion Replanning）**
   - 团队成员可提供建议触发Jeff重新规划
   - 支持针对特定任务或全局策略的优化建议
   - Jeff根据用户反馈智能调整执行计划
...
- **团队任务分配**：智能分配任务给Monica和Henry，优化团队效率
...
### 用户建议重新规划
```bash
curl -X POST "http://localhost:8000/api/v1/teams/{team_id}/planning/user-suggestion" \
  -H "Content-Type: application/json" \
  -H "X-User-ID: user123" \
  -d '{
    "suggestion_content": "建议优化当前的内容生成策略，增加更多互动元素和用户生成内容",
    "target_task_ids": ["task-001", "task-002"]
  }'
...
# 响应示例
{
  ...
  "replanning_task_id": "task-replan-001",
  "target_task_count": 2
}
...
curl -X POST "http://localhost:8000/api/v1/teams/{team_id}/planning/user-suggestion" \
  ...
  -H "X-User-ID: user123" \
  -d '{
    "suggestion_content": "团队工作效率有待提升，建议重新评估当前所有任务的优先级和分配策略"
  }'
```
...
### 团队领导者架构
- **Jeff（唯一Leader）**：每个团队只有一个Jeff实例，负责整体战略决策
- **Monica（可扩展）**：根据工作负载自动扩展1-5个实例
- **Henry（可扩展）**：根据审核需求自动扩展1-3个实例
